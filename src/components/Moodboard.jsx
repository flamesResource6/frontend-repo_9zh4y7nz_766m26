  ...
  return (
    <section className="bg-[#f6efe7] py-20 px-6">
      <div className="max-w-6xl mx-auto">
        <h3 className="font-serif text-3xl text-[#2b2a28] mb-6">The Palette of Purity</h3>
        <div className="flex gap-4 overflow-x-auto pb-4">
          {palette.map((p, i) => (
            <motion.div
              key={i}
              className="min-w-[260px] relative rounded-xl overflow-hidden shadow-sm"
              initial={{ opacity: 0, y: 10 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            >
              <img src={p.img} alt={p.label} className="w-[260px] h-48 object-cover"/>
              <div className="absolute bottom-0 left-0 right-0 p-3 text-sm text-[#2b2a28] bg-gradient-to-t from-[#f6efe7]/90 to-transparent">{p.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
